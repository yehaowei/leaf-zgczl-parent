...
A.Tree = Ext.extend(A.Component,{
	...
	initComponent:function(config){
		...
		if(sf.searchfield){
			sf.searchInput = $le(sf.id+'_search_field');
		}
	...
	initEvents:function(){
         ...
         * @event click
         * 点击事件.
         * @param {Leaf.Tree} Tree对象
         * @param {Leaf.Record} record 选中的Record对象
         * @param {Leaf.Tree.TreeNode} node 节点对象
         */
		EVT_CLICK,
		/**
         * @event dblclick
...
//			sf.dataset.locate.defer(5, sf.dataset,[sf.dataset.indexOf(node.record)+1,FALSE]);
	 ...
	 * @return {Leaf.Tree.TreeNode} node 节点对象
	 */
