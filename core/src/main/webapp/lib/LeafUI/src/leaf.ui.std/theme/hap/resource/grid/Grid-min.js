...
//        sf.gnb = wp.child('div.grid-nicescroll-box');
...
//        sf.fs = wp.child('a[atype=grid.focus]');
            ...
            sf.initTemplate();
            if(!Ext.isIE8){
                $jq("#" + sf.id).find("div.grid-nicescroll-box").niceScroll(".grid-nicescroll-wrap");
                $jq("#" + sf.id).find(".grid-ub").niceScroll({enablemousewheel:false});
            }
        },
...
//        Ext.getBody()[ou]('mousewheel',sf.onBodyMouseWheel,sf);
        ...
        initEvents:function(){
            ...
            this.addEvents(
                 ...
                 * 鼠标双击事件.
                 * @param {Leaf.Grid} grid 当前Grid组件.
                 * @param {Leaf.Record} record 鼠标双击所在行的Record对象.
                 * @param {Number} row 行号.
                 * @param {String} 当前name.
                 */
                EVT_DBLCLICK,
                 ...
                 * @param {Number} row 行号.
                 * @param {String} 当前name.
                 * @param {Leaf.Record} record 鼠标点击所在单元格的Record对象.
                 */
                EVT_CELL_CLICK,
                /**
                 * @event rowclick
                 ...
                 * @param {Leaf.Grid} grid 当前Grid组件.
                 * @param {Number} row 行号.
                 * @param {Leaf.Record} record 鼠标点击所在行的Record对象.
                 */
                EVT_ROW_CLICK,
                /**
                 * @event editorShow
        ...
        htmlUpdate : function(){
            if(!Ext.isIE8){
                $jq("html").getNiceScroll().resize();
            }
        },
        bodyUpdate : function(){
            var sf = this;
            if(!Ext.isIE8){
                $jq("#" + sf.id).find("div.grid-ub").getNiceScroll().resize();
            }
        },
...
//        if(focus !== FALSE) sf.focus.defer(10,sf);//获取数据后的获得焦点,会引起其他编辑器无法编辑
        ...
        customize : function(ctx){
            ...
            new Leaf.Window({id:'sys_customization_grid', url:context_path + 'modules/sys/sys_customization_grid.lview?source_file='+screen_path + '&id='+ this.id+'&did='+this.dataset.id, title:'个性化设置',height:530,width:560});
        },
...
//             if(!sf.hasFocus){
//                 if(!Ext.isIE8){
//                     var ns =$jq("#" + sf.id).find(".grid-ub").getNiceScroll(0);
//                     ns.doScrollTop(ns.scrollTop() + delta * (-28));
//                 }
...
//        var v = 0;
        ...
        renderEditor : function(div,record,c,editor){
            ...
            /*
             ...
             }else if(editor != _N || ($le(editor) instanceof CheckBox)){
        ...
        onUpdate : function(ds,record, name, value,oldvalue){
            ...
            if(div = Ext.get([sf.id,name,record.id].join(_))){
                ...
                if(editor!=_N ? ($le(editor) instanceof CheckBox):(name && !IS_EMPTY(record.getField(name).get(CHECKED_VALUE)))){
                    ...
                    c.showtitle && div.set({'title':$L.unescapeHtml(String(text).replace(/<[^<>]+>/mg,_N))});
                }
            }
            EACH(sf.columns,function(c){
                if(c.name != name && (ediv = Ext.get([sf.id,c.name,record.id].join(_)))) {
                    ...
                    if(c.renderer){
                        ...
                        c.showtitle && ediv.set({'title':$L.unescapeHtml(String(text).replace(/<[^<>]+>/mg,_N))});
                    }
                }
            });
            sf.setSelectStatus(record);
...
//        }
        ...
        setEditor: function(name,editor){
            ...
            if(div){
                if(editor == _N){
                    div.removeClass(CELL_EDITOR);
                }else if(!$le(editor) instanceof CheckBox){
                    div.addClass(CELL_EDITOR);
                }
            }
        },
...
//        });
        ...
        syncSize : function(){
            ...
            if(!Ext.isIE8){
                $jq("div.grid-ub").getNiceScroll().resize();
            }
        },
...
//    	sf.gnb.setHeight(bh);
            ...
            if(Ext.isChrome && !Ext.isIE8){
                $jq("div.grid-ub").getNiceScroll().remove();
                $jq("div.grid-ub").niceScroll({enablemousewheel:false});
                $jq("div.grid-ub").getNiceScroll().resize();
            }
        },
