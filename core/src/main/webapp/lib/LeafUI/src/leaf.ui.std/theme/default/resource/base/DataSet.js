...
$L.DataSet = Ext.extend(Ext.util.Observable,{
    ...
    destroy : function(){
        ...
        delete $L.invalidRecords[this.id]
    },
...
//        c.bindname = this.bindname;
    ...
    initFields : function(fields){
        for(var i = 0, len = fields.length; i < len; i++){
            var field = new $L.Record.Field(fields[i]);
...
//              }
        ...
        if(fire !== false) {
            ...
            if(!this.isValid) $L.showInfoMessage(_lang['dataset.info'], _lang['dataset.info.validate']);
        }
        return this.isValid;
    },
    /**
...
//            this.fireEvent("submit", this);
            $L.request({url:this.submiturl, para:p, ext:this.spara,success:this.onSubmitSuccess, error:this.onSubmitError, scope:this,failure:this.onAjaxFailed});
        //}
    },
    /**
     * 提交选中数据.
    ...
    post : function(url){
        ...
        if(!r)return;
        this.wait(true,function(){
    		if(this.validate())$L.post(url,r.data);
    	},this);
    },
    /**
     * 重置数据.
