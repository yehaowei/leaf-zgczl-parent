 ...
 */
$L.Window = Ext.extend($L.Component,{
	constructor: function(config) { 
		if($L.WindowManager.get(config.id))return;
        this.draggable = true;
        this.closeable = true;
        ...
        this.cmps = {};
//        $L.focusWindow = null;
        $L.Window.superclass.constructor.call(this,config);
    },
    initComponent : function(config){
    	$L.Window.superclass.initComponent.call(this, config);
    	var sf = this; 
    	$L.WindowManager.put(sf);
    	...
    	if(sf.fullScreen){
    		sf.width=$L.getViewportWidth()-(Ext.isIE||!sf.hasVScrollBar()?0:17)-(Ext.isIE8?1:0);
    		sf.height=$L.getViewportHeight()-(Ext.isIE||!sf.hasHScrollBar()?26:43);
    		sf.draggable = false;
    		sf.marginheight=1;
    		sf.marginwidth=1;
    	}
    ...
    processListener: function(ou){
    	$L.Window.superclass.processListener.call(this,ou);
    	if(this.closeable) {
    ...
    initEvents : function(){
    	$L.Window.superclass.initEvents.call(this);
    	this.addEvents(
    	/**
         * @event beforeclose
         * 窗口关闭前的事件.
...
//    	$L.focusWindow = this;    	
    ...
    onMouseDown : function(e){
		...
		sf.screenWidth = $L.getViewportWidth();
        sf.screenHeight = $L.getViewportHeight();
...
//    	$L.focusWindow = null;
    	...
    	if(this.modal) $L.Cover.uncover(this.wrap);
    	...
    	$L.Window.superclass.destroy.call(this);
    	delete this.title;
    	delete this.head;
    	delete this.body;
...
//    	}
    ...
    setWidth : function(w){
    	w=$L.getViewportWidth()-(Ext.isIE||!this.hasVScrollBar()?0:17)-(Ext.isIE8?1:0);
    	$L.Window.superclass.setWidth.call(this,w);
    	this.body.setWidth(w-2);
    	this.shadow.setWidth(this.wrap.getWidth());
    },
    setHeight : function(h){
    	h=$L.getViewportHeight()-(Ext.isIE||!this.hasHScrollBar()?26:43);
 ...
 * @return {Window} 窗口对象
 */
$L.showMessage = function(title, msg,callback,width,height){
	return $L.showTypeMessage(title, msg, width||300, height||100,'win-info',callback);
}
/**
 * 显示带警告图标的窗口
 * 
 ...
 * @return {Window} 窗口对象
 */
$L.showWarningMessage = function(title, msg,callback,width,height){
	return $L.showTypeMessage(title, msg, width||300, height||100,'win-warning',callback);
}
/**
 * 显示带信息图标的窗口
 * 
 ...
 * @return {Window} 窗口对象
 */
$L.showInfoMessage = function(title, msg,callback,width,height){
	return $L.showTypeMessage(title, msg, width||300, height||100,'win-info',callback);
}
/**
 * 显示带错误图标的窗口
 ...
 * @return {Window} 窗口对象
 */
$L.showErrorMessage = function(title,msg,callback,width,height){
	return $L.showTypeMessage(title, msg, width||300, height||100,'win-error',callback);
}

$L.showTypeMessage = function(title, msg,width,height,css,callback){
	var msg = '<div class="win-icon '+css+'"><div class="win-type" style="width:'+(width-60)+'px;height:'+(height-62)+'px;">'+msg+'</div></div>';
	return $L.showOkWindow(title, msg, width, height,callback);	
} 
/**
 * 带图标的确定窗口.
 * 
 ...
 * @return {Window} 窗口对象
 */
$L.showConfirm = function(title, msg, okfun,cancelfun, width, height){
	width = width||300;
	height = height||100;
    var msg = '<div class="win-icon win-question"><div class="win-type" style="width:'+(width-60)+'px;height:'+(height-62)+'px;">'+msg+'</div></div>';
    return $L.showOkCancelWindow(title, msg, okfun,cancelfun, width, height);  	
}
//$L.hideWindow = function(){
//	var cmp = $L.CmpManager.get('leaf-msg')
//	if(cmp) cmp.close();
//}
//$L.showWindow = function(title, msg, width, height, cls){
//	cls = cls ||'';
//	var cmp = $L.CmpManager.get('leaf-msg')
//	if(cmp == null) {
//		cmp = new $L.Window({id:'leaf-msg',title:title, height:height,width:width});
//		if(msg){
 ...
 * @return {Window} 窗口对象
 */
$L.showOkCancelWindow = function(title, msg, okfun,cancelfun,width, height){
    //var cmp = $L.CmpManager.get('leaf-msg-ok-cancel')
    //if(cmp == null) {
        var id = Ext.id(),okid = 'leaf-msg-ok'+id,cancelid = 'leaf-msg-cancel'+id,
        okbtnhtml = $L.Button.getTemplate(okid,_lang['window.button.ok']),
        cancelbtnhtml = $L.Button.getTemplate(cancelid,_lang['window.button.cancel']),
        cmp = new $L.Window({id:'leaf-msg-ok-cancel'+id,closeable:true,title:title, height:height||100,width:width||300});
        if(msg){
 ...
 * @return {Window} 窗口对象
 */
$L.showOkWindow = function(title, msg, width, height,callback){
	//var cmp = $L.CmpManager.get('leaf-msg-ok');
	//if(cmp == null) {
		var id = Ext.id(),yesid = 'leaf-msg-yes'+id,
		btnhtml = $L.Button.getTemplate(yesid,_lang['window.button.ok']),
		cmp = new $L.Window({id:'leaf-msg-ok'+id,closeable:true,title:title, height:height,width:width});
		if(msg){
 ...
 */
$L.showUploadWindow = function(path,title,source_type,pkvalue,max_size,file_type,callback){
