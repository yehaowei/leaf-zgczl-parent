...
var AUTO_ID=0,
    ...
    getColumnData = function(type,index,pid,width){
        var opt = Options.get(type),
            data = {};
        Ext.iterate(globalConfig,function(key,value){
            if(Ext.isDefined(opt[key])){
        ...
        data[TAB_FIELD_TABTYPE] = Types.find(type);
        /*data[TAB_FIELD_TABCODE] = 'editor-'+(++AUTO_ID);*/
        data['column_name'] = 'column-'+(++AUTO_ID);
        if(Ext.isDefined(pid)){
            data[TAB_FIELD_TABCODE] = pid;
            data[TAB_FIELD_PARENTTABCODE] = pid;
            data[TAB_FIELD_SETUPWIDTH] = 'ABSOLUTE_WIDTH';
        }
        if(type == 'TabPanel'){
	...
	getConfig = function(record){
		var config = {
			record : record,
			dataset : record.ds,
			childsdataset:TAB_COLUMN_DS
		};
		Ext.iterate(globalConfig,function(key,value){
			config[key] = record.get(value);
		});
		if(record.get("column_name") != null){
			config.id = record.get("tab_code") + '_' + record.get("column_name");
		}
		//tab组件设置tab_group
...
//						sf.setHeight(parseInt(ol.height));
			...
			close : function(e){
				var sf = this,
					xy = e.xy,
					win=$L.showConfirm('警告','是否删除当前'+sf.type+'控件？',sf.onClose.createDelegate(sf),function(){
						sf.focus();
					}),
					vw = $L.getViewportWidth(),
					w = win.wrap.getWidth(),
					x = xy[0],y;
				if(e.type == 'keydown'){
			...
			//初始化右侧信息栏
			initComponentAttributes : function(record){
                if(record.data.tab_type == "TEXTFIELD" || record.data.tab_type == "DATEPICKER" || record.data.tab_type == "LOV" || record.data.tab_type == 'COMBOBOX' || record.data.tab_type == 'PERCENTFIELD' || record.data.tab_type == undefined){
                    jQuery("#layoutInfo").hide();
                    jQuery("#baseInfo").show();
                    TAB_FORM_COLUMN_DS.removeAll();
                    TAB_FORM_COLUMN_DS.create(record.data);
				}else {
                    jQuery("#baseInfo").hide();
                    jQuery("#layoutInfo").show();
                    TAB_FORM_DS.removeAll();
                    TAB_FORM_DS.create(record.data);
                }
                jQuery(".editor-tab-attributes").animate({width:"330px",opacity:'1'});
			}
		});
	}(),
	Input = Ext.extend(Observable,{
        ...
        onClick:function(e,t){
            this.initComponentAttributes(this.record);
            stopEvent(e);
        }
	}),
	Box = function(){
...
//				},
			...
			updatePrompt : function(target){
                ...
                var datas = sf.childsdataset.getAll();
                for(var i = 0; i < datas.length; i++){
                	var str = datas[i].get('tab_code') + '_' + datas[i].get('column_name');
                    if(str == target.child(".text-content").dom.getAttribute("data-id")){
                        datas[i].set('prompt',  (text||binder.emptytext));
                    }
                }
			...
			onClick : function(e,t){
                ...
                this.initComponentAttributes(this.getRecord());
			},
			onDrag : function(){
				var sf = this;
			...
			onDrop : function(e,t){
				...
				if(Proxy.isShow()){
					if(Ext.isString(DRAG_TYPE)){
						if(DRAG_TYPE == 'TextField' || DRAG_TYPE == 'DatePicker' || DRAG_TYPE == 'ComboBox' || DRAG_TYPE == 'PercentField' || DRAG_TYPE == 'Lov'){
                            sf.childsdataset.create(getColumnData(DRAG_TYPE, dropIndex, sf.id), dropIndex);
						}else {
			...
			create : function(record,index){
				var sf = this,
					type = Types.get(record.get(TAB_FIELD_TABTYPE) || record.get("validation_type"));
				if(!type)return;
					...
					for(var i=0;i<rows;i++){
						...
						for(var j=0;j<columns;j++){
				    		...
				    		if(child){
				    			...
				    			if(child.prompt||child.type=='TextField' || child.type == 'DatePicker' || child.type == 'ComboBox' || child.type == 'PercentField' || child.type == 'Lov'){
								...
								if(child === proxy){
					    			...
					    			th.set({'data-childindex':index}).child('.text-content').update(child.prompt||child.getRecord().get("prompt")||'Prompt');
                                    th.child('.text-content').dom.setAttribute("data-id",child.record.data.tab_code+'_'+child.record.data.column_name);
...
//				});
	...
	pub={
        ...
        DatePicker : Ext.extend(Input,{
            type:'DatePicker',
            wrapTplt : ['<DIV tabindex="0" class="item-tf item-wrap editor-wrap" style="width:{width}px;height:{height}px;" >',
                '<DIV class="item-input-wrap">',
                '<INPUT  style="height:{height}px;text-indent:2px;" disabled class="item-textField" />',
                '</DIV>',
				'<DIV class="item-trigger item-dateButton">',
				'</DIV>',
                '</DIV>']
        }),
        Lov : Ext.extend(Input,{
			type:'Lov',
			wrapTplt : ['<DIV tabindex="0" class="item-tf item-wrap editor-wrap" style="width:{width}px;height:{height}px;" >',
				'<DIV class="item-input-wrap">',
				'<INPUT  style="height:{height}px;text-indent:2px;" disabled class="item-textField" />',
				'</DIV>',
				'<DIV class="item-trigger item-lovButton">',
				'</DIV>',
				'</DIV>']
		}),
        ComboBox :  Ext.extend(Input,{
			type:'ComboBox',
			wrapTplt : ['<DIV tabindex="0" class="item-tf item-wrap editor-wrap" style="width:{width}px;height:{height}px;" >',
				'<DIV class="item-input-wrap">',
				'<INPUT  style="height:{height}px;text-indent:2px;" disabled class="item-textField" />',
				'</DIV>',
				'<DIV class="item-trigger item-comboButton">',
				'</DIV>',
				'</DIV>']
    	}),
        PercentField :  Ext.extend(Input,{
            type:'PercentField',
            wrapTplt : ['<DIV tabindex="0" class="item-tf item-wrap editor-wrap" style="width:{width}px;height:{height}px;" >',
                '<DIV class="item-input-wrap">',
                '<INPUT  style="height:{height}px;text-indent:2px;" disabled class="item-textField" />',
                '</DIV>',
                '<DIV class="item-trigger item-percent-icon">',
                '</DIV>',
                '</DIV>']
        }),
		VBox : Ext.extend(Box,{
		...
		Form : Ext.extend(Box,{
						...
						'<thead>',
							'<tr>',
								'<th class="form_head empty-text">',
									'<div class="text-content layout-th">Title</div>',
			...
			initComponent : function(config){
				...
				sf.title = sf.wrap.child(".text-content");
                sf.title.dom.innerHTML = config.title || 'Title';
            },
			redraw : function(drop){
...
//				},
				...
				onDblClick : function(e){
					...
					if(sf.wrap.hasClass('can-col-resize')){
						...
						!resizeLabel && next && $L.showConfirm('警告','是否删除右侧列？',function(){
		...
		Grid : function(){
			...
			return Ext.extend(Observable,{
				...
				createColumn : function(width){
					...
					//创建列
					var record = sf.childsdataset.create(getColumnData("column", sf.unlockcolumns.length, sf.id,width), sf.unlockcolumns.length);
					col.tab_code = record.data.tab_code + '_' + record.data.column_name;
                    col.record = record;
                    col.promptEl.child("div").dom.setAttribute("data-id",col.tab_code);
                    sf.uh.child("table").id = col.tab_code;
                    ElementManager.put(sf.uh.child("table"));
					uhl.child('[atype=fixed-cell]').appendTo(uhl);
					sf.syncColumnSize();
					sf.showEditor(col);
				},
				initColumn : function(record){
                    var sf = this,
                        uhl = sf.uh.child('.grid-hl'),
                        col;
                    sf.unlockcolumns.push(col={
                    ...
                    ElementManager.put(sf.uh.child("table"));
                    col.promptEl.child("div").dom.setAttribute("data-id",col.tab_code);
                    col.promptEl.child("div").update(record.get("prompt"));
                    uhl.child('[atype=fixed-cell]').appendTo(uhl);
                    sf.syncColumnSize();
                    sf.showEditor(col);
                    return sf;
				...
				syncColumnSize : function(islock){
					...
					if(islock){
						...
						EACH(sf.unlockcolumns,function(col){
							col.record.data.width = col.width;
							w+=col.width;
						});
						sf.unlockwidth = w;
					}
				...
				bindEditor : function(col){
                    ...
                    ed.el.dom.setAttribute("data-id",col.tab_code);
					(ed.setXY(el.getXY())
				...
				updatePrompt : function(col){
					...
					this.getRecordByCode(this.childsdataset.getAll(),col.promptEl.child("div").getAttribute("data-id")).set("prompt",this.editor.el.dom.value);
				},
				...
				create : function(record){
					if(record.get("config_id") != null) {
                        return this.initColumn(record);
                    }
				},
				...
				onClick : function(e,t){
					...
					if(t.hasClass('grid-hc')||(t = t.parent('.grid-hc'))){
						...
						EACH(sf.getColumns(),function(col){
							if(col.promptEl == t){
								sf.showEditor(col);
                                sf.initComponentAttributes(sf.getRecordByCode(sf.childsdataset.getAll(),col.promptEl.child("div").getAttribute("data-id")));
								return false;
							}
						})
                    }else if(e.target.className == "item-textField"){
                        this.initComponentAttributes(this.getRecordByCode(this.childsdataset.getAll(),e.target.getAttribute("data-id")));
					}else{
                        this.initComponentAttributes(this.getRecord());
					}
				},
				getRecordByCode:function(record,code){
					for(var i = 0; i < record.length; i++){
						var str = record[i].data.tab_code + '_' + record[i].data.column_name;
						if(str == code){
							return record[i];
						}
					}
				...
				Tab = Ext.extend(Abstract,{
					...
					initComponent : function(config){
						...
						sf.strip = new Ext.Template(stripTpl).append(sf.head,{
							...
							title:sf.title|| config.title ||''
						},true);
						sf.strip.on("click",sf.stripClick,sf);
                        sf.strip.child(".tab-close").on("click",sf.remove,sf);
						sf.body = (sf.wrap = new Ext.Template(bodyTpl).append(sf.wrapbody,{},true)).child('table');
						sf.children=[];
					},
                    stripClick : function(){
                        this.parent.initComponentAttributes(this.getRecord());
					},
					remove : function(e){
						var sf = this;
						sf.strip.remove();
                        sf.dataset.remove(sf.getRecord());
                        sf.parent.onTabStripResize();
                        stopEvent(e);
					},
...
function dataSetUpdate(ds,record,name,value,old_value){
	if(record.data.tab_code != null && record.data.tab_code != ""){
		var data = TAB_DS.getAll();
		for(var i = 0; i < data.length; i++){
			if(data[i].data.tab_code == record.data.tab_code){
                data[i].data[name] = value;
                TAB_DS.fireEvent('update',TAB_DS,data[i],name,value,old_value);
                return;
			}
		}
	}
};
function columnDataSetUpdate(ds,record,name,value,old_value){
	if(record.data.tab_code != null && record.data.tab_code != ""){
		var data = TAB_COLUMN_DS.getAll();
		for(var i = 0; i < data.length; i++){
            var dataFlag = data[i].data.tab_code+'_'+data[i].data.column_name;
            var recordFlag = record.data.tab_code+'_'+record.data.column_name;
			if(dataFlag == recordFlag){
				data[i].data[name] = value;
                TAB_COLUMN_DS.fireEvent('update',TAB_COLUMN_DS,data[i],name,value,old_value);
				return;
			}
		}
	}
};
$L.ScreenEditor = function(){
	var dropIndex = -1,
	...
	return Ext.extend($L.Component, {
		...
		bind : function(ds,columnds,formDs,formColumnDs){
			if(Ext.isString(ds)){
	            ds = $(ds);
	            if(!ds) return;
	        }
	        ...
	        if(Ext.isString(formDs)){
                formDs = $(formDs);
                if(!formDs) return;
			}
            if(Ext.isString(formColumnDs)){
                formColumnDs = $(formColumnDs);
                if(!formColumnDs) return;
            }
	        var sf = this;
	        TAB_DS = ds;
	        TAB_COLUMN_DS = columnds;
	        TAB_FORM_DS = formDs;
            TAB_FORM_DS.on('update',dataSetUpdate);
            TAB_FORM_COLUMN_DS = formColumnDs;
            TAB_FORM_COLUMN_DS.on('update',columnDataSetUpdate);
...
//	            ds[ou]('afterwait', sf.onAfterSuccess, sf);
		...
		onAdd : function(ds,record,index){
            var str = record.get(TAB_FIELD_PARENTTABCODE);
	    	var group = record.get(TAB_FIELD_TABGROUP),
	    		parent = (group?tab_groups[group]:ElementManager.get(str))||this;
	    	parent.create(record,index);
	    },
	    onRemove : function(ds,record){
            var str = record.get(TAB_FIELD_TABCODE);
            if(record.get("column_name") != null){
                str = str + '_' + record.get("column_name");
            }
	    	var el = ElementManager.get(str);
	    	el.destroy();
	    },
	    onUpdate : function(ds,record,name,value,oldvalue){
	    	...
	    	var el = ElementManager.get(str);
	    	if(el != null && el.type == 'Tab'){
				el = el.panel;	    		
	    	}
	    	switch(name){
				case 'column_name':
					//修改管理器已存id
					var newStr = record.data.tab_code + '_' + value;
					var oldStr = record.data.tab_code + '_' + oldvalue;
                    ElementManager.updateKey(oldStr,newStr);
					//修改已存在div的data-id
                    var obj = jQuery("div[data-id='"+oldStr+"']")[0];
                    obj.setAttribute("data-id",newStr);
					break;
				case 'prompt':
					var str = record.data.tab_code + '_' + record.data.column_name;
                    jQuery("div[data-id='"+str+"']").html(value);
					break;
				case TAB_DESC:
                    el = ElementManager.get(str);
					if(record.get("tab_type") == 'TAB'){
                        el.strip.child(".strip-center").dom.textContent = value;
					}else {
                        el.title.dom.innerText = value;
                    }
					break;
	    ...
	    onLoad : function(){
	    	var sf = this,
	    		datas = TAB_DS.getAll(),
				dataColumns = TAB_COLUMN_DS.getAll();
    		sf.clear();
	    	if(datas.length){
		    	var map1 = {},map2 = {},array = [];
		    	...
		    	EACH(dataColumns,function(r){
                    var id = r.get(TAB_FIELD_TABCODE);
					if(r.get('column_name') != null){
						id +=   '_' + r.get('column_name');
					}
                    r.set('parent_tab_code',r.get('tab_code'));
                    map1[id] = map2[id]={
                        record:r,
                        children:[]
                    };
	    ...
	    onIndexChange : function(ds,r){
	    	...
	    	if(type == 'TabPanel'){
	    		...
	    		var str = r.get(TAB_FIELD_TABCODE);
	    		if(r.get("column_name") != null){
	    			str += '_' + r.get("column_name");
				}
		    	var el = ElementManager.get(str);
		    	el && el.focus();
	    	}
	    ...
	    createElements : function(arr,parent){
	    	...
	    	EACH(arr,function(node){
	    		...
	    		if(record.get('enabled_flag') == 'Y'){
	    			...
	    			if(el.type != null && el.type == 'TabPanel'){
	    				parent = el;
	    				el = el.currentTab;
	    			}
		...
		setScreenSize : function(size){
			var wh = size.split('*');
			screenWrap.setStyle({
				width : (screenwidth = Number(wh[0])) - 10 + 'px',
