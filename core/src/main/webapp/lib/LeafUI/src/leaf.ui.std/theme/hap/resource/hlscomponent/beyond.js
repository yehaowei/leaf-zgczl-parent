/*Sets Themed Colors Based on Themes*/

var themeprimary = getThemeColorFromCss('themeprimary');
var themesecondary = getThemeColorFromCss('themesecondary');
var themethirdcolor = getThemeColorFromCss('themethirdcolor');
var themefourthcolor = getThemeColorFromCss('themefourthcolor');
var themefifthcolor = getThemeColorFromCss('themefifthcolor');

//Gets Theme Colors From Selected Skin To Use For Drawing Charts
function getThemeColorFromCss(style) {
    var $span = $jq("<span></span>").hide().appendTo("body");
    $span.addClass(style);
    var color = $span.css("color");
    $span.remove();
    return color;
}

//Handle RTL SUpport for Changer CheckBox
$jq("#skin-changer li a").click(function () {
    createCookie("current-skin", $jq(this).attr('rel'), 10);
    window.location.reload();
});

//Checks Not to Do rtl-support for Arabic and Persian Demo Pages

var rtlchanger = document.getElementById('rtl-changer');

if (location.pathname != "/index-rtl-fa.html" && location.pathname != "/index-rtl-ar.html") {
    if (readCookie("rtl-support")) {
        switchClasses("pull-right", "pull-left");
        switchClasses("databox-right", "databox-left");
        switchClasses("item-right", "item-left");
        $jq('.navbar-brand small img').attr('src', 'assets/img/logo-rtl.png');
        if (rtlchanger != null)
            document.getElementById('rtl-changer').checked = true;
    }
    else {
        if (rtlchanger != null)
            rtlchanger.checked = false;
    }

    if (rtlchanger != null) {
        rtlchanger.onchange = function () {
            if (this.checked) {
                createCookie("rtl-support", "true", 10);
            }
            else {
                eraseCookie("rtl-support");
            }
            setTimeout(function () {
                window.location.reload();
            }, 600);

        };
    }
}
/*Loading*/
$jq(window)
    .load(function () {
        setTimeout(function () {
            $jq('.loading-container')
                .addClass('loading-inactive');
        }, 0);
    });


/*Account Area --> Setting Button*/
$jq('#btn-setting')
    .on('click', function (e) {
        $jq('.navbar-account')
            .toggleClass('setting-open');
    });

/*Toggle FullScreen*/
$jq('#fullscreen-toggler')
    .on('click', function (e) {
        var element = document.documentElement;
        if (!$jq('body')
            .hasClass("full-screen")) {

            $jq('body')
                .addClass("full-screen");
            $jq('#fullscreen-toggler')
                .addClass("active");
            if (element.requestFullscreen) {
                element.requestFullscreen();
            } else if (element.mozRequestFullScreen) {
                element.mozRequestFullScreen();
            } else if (element.webkitRequestFullscreen) {
                element.webkitRequestFullscreen();
            } else if (element.msRequestFullscreen) {
                element.msRequestFullscreen();
            }

        } else {

            $jq('body')
                .removeClass("full-screen");
            $jq('#fullscreen-toggler')
                .removeClass("active");

            if (document.exitFullscreen) {
                document.exitFullscreen();
            } else if (document.mozCancelFullScreen) {
                document.mozCancelFullScreen();
            } else if (document.webkitExitFullscreen) {
                document.webkitExitFullscreen();
            }

        }
    });

/*Handles Popovers*/
var popovers = $jq('[data-toggle=popover]');
$jq.each(popovers, function () {
    $jq(this)
        .popover({
            html: true,
            template: '<div class="popover ' + $jq(this)
                .data("class") +
                '"><div class="arrow"></div><h3 class="popover-title ' +
                $jq(this)
                .data("titleclass") + '">Popover right</h3><div class="popover-content"></div></div>'
        });
});

var hoverpopovers = $jq('[data-toggle=popover-hover]');
$jq.each(hoverpopovers, function () {
    $jq(this)
        .popover({
            html: true,
            template: '<div class="popover ' + $jq(this)
                .data("class") +
                '"><div class="arrow"></div><h3 class="popover-title ' +
                $jq(this)
                .data("titleclass") + '">Popover right</h3><div class="popover-content"></div></div>',
            trigger: "hover"
        });
});


/*Handles ToolTips*/
$jq("[data-toggle=tooltip]")
    .tooltip({
        html: true
    });

InitiateSideMenu();
InitiateSettings();
InitiateWidgets();

function InitiateSideMenu() {

    //Sidebar Toggler
    $jq(".sidebar-toggler").on('click', function () {
        $jq("#sidebar").toggleClass("hide");
        $jq(".sidebar-toggler").toggleClass("active");
        return false;
    });
    //End Sidebar Toggler

    //Sidebar Collapse
    var b = $jq("#sidebar").hasClass("menu-compact");
    $jq("#sidebar-collapse").on('click', function () {
        if (!$jq('#sidebar').is(':visible'))
            $jq("#sidebar").toggleClass("hide");
        $jq("#sidebar").toggleClass("menu-compact");
        $jq(".sidebar-collapse").toggleClass("active");
        b = $jq("#sidebar").hasClass("menu-compact");

        if ($jq(".sidebar-menu").closest("div").hasClass("slimScrollDiv")) {
            $jq(".sidebar-menu").slimScroll({ destroy: true });
            $jq(".sidebar-menu").attr('style', '');
        }
        if (b) {
            $jq(".open > .submenu")
                .removeClass("open");
        } else {
            if ($jq('.page-sidebar').hasClass('sidebar-fixed')) {
                var position = (readCookie("rtl-support") || location.pathname == "/index-rtl-fa.html" || location.pathname == "/index-rtl-ar.html") ? 'right' : 'left';
                $jq('.sidebar-menu').slimscroll({
                    height: 'auto',
                    position: position,
                    size: '3px',
                    color: themeprimary
                });
            }
        }
        //Slim Scroll Handle



    });
    //End Sidebar Collapse


    //Sidebar Menu Handle
    $jq(".sidebar-menu").on('click', function (e) {
        var menuLink = $jq(e.target).closest("a");
        if (!menuLink || menuLink.length == 0)
            return;
        if (!menuLink.hasClass("menu-dropdown")) {
            if (b && menuLink.get(0).parentNode.parentNode == this) {
                var menuText = menuLink.find(".menu-text").get(0);
                if (e.target != menuText && !$.contains(menuText, e.target)) {
                    return false;
                }
            }
            return;
        }
        var submenu = menuLink.next().get(0);
        if (!$jq(submenu).is(":visible")) {
            var c = $jq(submenu.parentNode).closest("ul");
            if (b && c.hasClass("sidebar-menu"))
                return;
            c.find("> .open > .submenu")
                .each(function () {
                    if (this != submenu && !$jq(this.parentNode).hasClass("active"))
                        $jq(this).slideUp(200).parent().removeClass("open");
                });
        }
        if (b && $jq(submenu.parentNode.parentNode).hasClass("sidebar-menu"))
            return false;
        $jq(submenu).slideToggle(200).parent().toggleClass("open");
        return false;
    });
    //End Sidebar Menu Handle
}

function InitiateWidgets() {
    $jq('.widget-buttons *[data-toggle="maximize"]').on("click", function (event) {
        event.preventDefault();
        var widget = $jq(this).parents(".widget").eq(0);
        var button = $jq(this).find("i").eq(0);
        var compress = "fa-compress";
        var expand = "fa-expand";
        if (widget.hasClass("maximized")) {
            if (button) {
                button.addClass(expand).removeClass(compress);
            }
            widget.removeClass("maximized");
            widget.find(".widget-body").css("height", "auto");
        } else {
            if (button) {
                button.addClass(compress).removeClass(expand);
            }
            widget.addClass("maximized");
            maximize(widget);
        }
    });

    $jq('.widget-buttons *[data-toggle="collapse"]').on("click", function (event) {
        event.preventDefault();
        var widget = $jq(this).parents(".widget").eq(0);
        var body = widget.find(".widget-body");
        var button = $jq(this).find("i");
        var down = "fa-plus";
        var up = "fa-minus";
        var slidedowninterval = 300;
        var slideupinterval = 200;
        if (widget.hasClass("collapsed")) {
            if (button) {
                button.addClass(up).removeClass(down);
            }
            widget.removeClass("collapsed");
            body.slideUp(0, function () {
                body.slideDown(slidedowninterval);
            });
        } else {
            if (button) {
                button.addClass(down)
                    .removeClass(up);
            }
            body.slideUp(slideupinterval, function () {
                widget.addClass("collapsed");
            });
        }
    });

    $jq('.widget-buttons *[data-toggle="dispose"]').on("click", function (event) {
        event.preventDefault();
        var toolbarLink = $jq(this);
        var widget = toolbarLink.parents(".widget").eq(0);
        var disposeinterval = 300;
        widget.hide(disposeinterval, function () {
            widget.remove();
        });
    });
}

// Fullscreen Widget
function maximize(widgetbox) {
    if (widgetbox) {
        var windowHeight = $jq(window).height();
        var headerHeight = widgetbox.find(".widget-header").height();
        widgetbox.find(".widget-body").height(windowHeight - headerHeight);
    }
}

/* Scroll To */
function scrollTo(el, offeset) {
    var pos = (el && el.size() > 0) ? el.offset().top : 0;
    jQuery('html,body').animate({ scrollTop: pos + (offeset ? offeset : 0) }, 'slow');
}

/*Show Notification*/
function Notify(message, position, timeout, theme, icon, closable) {
    toastr.options.positionClass = 'toast-' + position;
    toastr.options.extendedTimeOut = 0; //1000;
    toastr.options.timeOut = timeout;
    toastr.options.closeButton = closable;
    toastr.options.iconClass = icon + ' toast-' + theme;
    toastr['custom'](message);
}

/*#region handle Settings*/
function InitiateSettings() {
    if (readCookie("navbar-fixed-top") != null) {
        if (readCookie("navbar-fixed-top") == "true") {
            $jq('#checkbox_fixednavbar').prop('checked', true);
            $jq('.navbar').addClass('navbar-fixed-top');
        }
    }

    if (readCookie("sidebar-fixed") != null) {
        if (readCookie("sidebar-fixed") == "true") {
            $jq('#checkbox_fixedsidebar').prop('checked', true);
            $jq('.page-sidebar').addClass('sidebar-fixed');

            //Slim Scrolling for Sidebar Menu in fix state
            if (!$jq(".page-sidebar").hasClass("menu-compact")) {
                var position = (readCookie("rtl-support") || location.pathname == "/index-rtl-fa.html" || location.pathname == "/index-rtl-ar.html") ? 'right' : 'left';
                $jq('.sidebar-menu').slimscroll({
                    height: $jq(window).height() - 90,
                    position: position,
                    size: '3px',
                    color: themeprimary
                });
            }
        }

    }
    if (readCookie("breadcrumbs-fixed") != null) {
        if (readCookie("breadcrumbs-fixed") == "true") {
            $jq('#checkbox_fixedbreadcrumbs').prop('checked', true);
            $jq('.page-breadcrumbs').addClass('breadcrumbs-fixed');
        }
    }
    if (readCookie("page-header-fixed") != null) {
        if (readCookie("page-header-fixed") == "true") {
            $jq('#checkbox_fixedheader').prop('checked', true);
            $jq('.page-header').addClass('page-header-fixed');
        }
    }


    $jq('#checkbox_fixednavbar')
        .change(function () {
            $jq('.navbar')
                .toggleClass('navbar-fixed-top');

            if (($jq('#checkbox_fixedsidebar')
                .is(":checked"))) {
                $jq('#checkbox_fixedsidebar')
                    .prop('checked', false);
                $jq('.page-sidebar')
                    .toggleClass('sidebar-fixed');
            }

            if (($jq('#checkbox_fixedbreadcrumbs')
                .is(":checked")) && !($jq(this)
                .is(":checked"))) {
                $jq('#checkbox_fixedbreadcrumbs')
                    .prop('checked', false);
                $jq('.page-breadcrumbs')
                    .toggleClass('breadcrumbs-fixed');
            }

            if (($jq('#checkbox_fixedheader')
                .is(":checked")) && !($jq(this)
                .is(":checked"))) {
                $jq('#checkbox_fixedheader')
                    .prop('checked', false);
                $jq('.page-header')
                    .toggleClass('page-header-fixed');
            }
            setCookiesForFixedSettings();
        });

    $jq('#checkbox_fixedsidebar')
        .change(function () {

            $jq('.page-sidebar')
                .toggleClass('sidebar-fixed');

            if (!($jq('#checkbox_fixednavbar')
                .is(":checked"))) {
                $jq('#checkbox_fixednavbar')
                    .prop('checked', true);
                $jq('.navbar')
                    .toggleClass('navbar-fixed-top');
            }
            if (($jq('#checkbox_fixedbreadcrumbs')
                .is(":checked")) && !($jq(this)
                .is(":checked"))) {
                $jq('#checkbox_fixedbreadcrumbs')
                    .prop('checked', false);
                $jq('.page-breadcrumbs')
                    .toggleClass('breadcrumbs-fixed');
            }

            if (($jq('#checkbox_fixedheader')
                .is(":checked")) && !($jq(this)
                .is(":checked"))) {
                $jq('#checkbox_fixedheader')
                    .prop('checked', false);
                $jq('.page-header')
                    .toggleClass('page-header-fixed');
            }
            setCookiesForFixedSettings();

        });
    $jq('#checkbox_fixedbreadcrumbs')
        .change(function () {

            $jq('.page-breadcrumbs')
                .toggleClass('breadcrumbs-fixed');


            if (!($jq('#checkbox_fixedsidebar')
                .is(":checked"))) {
                $jq('#checkbox_fixedsidebar')
                    .prop('checked', true);
                $jq('.page-sidebar')
                    .toggleClass('sidebar-fixed');
            }
            if (!($jq('#checkbox_fixednavbar')
                .is(":checked"))) {
                $jq('#checkbox_fixednavbar')
                    .prop('checked', true);
                $jq('.navbar')
                    .toggleClass('navbar-fixed-top');
            }
            if (($jq('#checkbox_fixedheader')
                .is(":checked")) && !($jq(this)
                .is(":checked"))) {
                $jq('#checkbox_fixedheader')
                    .prop('checked', false);
                $jq('.page-header')
                    .toggleClass('page-header-fixed');
            }
            setCookiesForFixedSettings();

        });

    $jq('#checkbox_fixedheader')
        .change(function () {

            $jq('.page-header')
                .toggleClass('page-header-fixed');


            if (!($jq('#checkbox_fixedbreadcrumbs')
                .is(":checked"))) {
                $jq('#checkbox_fixedbreadcrumbs')
                    .prop('checked', true);
                $jq('.page-breadcrumbs')
                    .toggleClass('breadcrumbs-fixed');
            }

            if (!($jq('#checkbox_fixedsidebar')
                .is(":checked"))) {
                $jq('#checkbox_fixedsidebar')
                    .prop('checked', true);
                $jq('.page-sidebar')
                    .toggleClass('sidebar-fixed');
            }
            if (!($jq('#checkbox_fixednavbar')
                .is(":checked"))) {
                $jq('#checkbox_fixednavbar')
                    .prop('checked', true);
                $jq('.navbar')
                    .toggleClass('navbar-fixed-top');
            }

            setCookiesForFixedSettings();
        });
}

function setCookiesForFixedSettings() {
    createCookie("navbar-fixed-top", $jq('#checkbox_fixednavbar').is(':checked'), 100);
    createCookie("sidebar-fixed", $jq('#checkbox_fixedsidebar').is(':checked'), 100);
    createCookie("breadcrumbs-fixed", $jq('#checkbox_fixedbreadcrumbs').is(':checked'), 100);
    createCookie("page-header-fixed", $jq('#checkbox_fixedheader').is(':checked'), 100);

    var position = (readCookie("rtl-support") || location.pathname == "/index-rtl-fa.html" || location.pathname == "/index-rtl-ar.html") ? 'right' : 'left';
    if ($jq('#checkbox_fixedsidebar').is(':checked')) {
        if (!$jq('.page-sidebar').hasClass('menu-compact')) {
            //Slim Scrolling for Sidebar Menu in fix state
            $jq('.sidebar-menu').slimscroll({
                position: position,
                size: '3px',
                color: themeprimary,
                height: $jq(window).height() - 90,
            });
        }
    } else {
        if ($jq(".sidebar-menu").closest("div").hasClass("slimScrollDiv")) {
            $jq(".sidebar-menu").slimScroll({ destroy: true });
            $jq(".sidebar-menu").attr('style', '');
        }
    }
}
/*#endregion handle Settings*/

//Chat
$jq("#chat-link").click(function () {
    $jq('.page-chatbar').toggleClass('open');
    $jq("#chat-link").toggleClass('open');
});
$jq('.page-chatbar .chatbar-contacts .contact').on('click', function (e) {
    $jq('.page-chatbar .chatbar-contacts').hide();
    $jq('.page-chatbar .chatbar-messages').show();
});

$jq('.page-chatbar .chatbar-messages .back').on('click', function (e) {
    $jq('.page-chatbar .chatbar-contacts').show();
    $jq('.page-chatbar .chatbar-messages').hide();
});
var position = (readCookie("rtl-support") || location.pathname == "/index-rtl-fa.html" || location.pathname == "/index-rtl-ar.html") ? 'right' : 'left';
var additionalHeight = 0;
if ($jq(window).width() < 531)
    additionalHeight = 45;
$jq('.chatbar-messages .messages-list').slimscroll({
    position: position,
    size: '4px',
    color: themeprimary,
    height: $jq(window).height() - (250 + additionalHeight),
});
$jq('.chatbar-contacts .contacts-list').slimscroll({
    position: position,
    size: '4px',
    color: themeprimary,
    height: $jq(window).height() - (86 + additionalHeight),
});
//End Chat

/*#region Get Colors*/
//Get colors from a string base on theme colors
function getcolor(colorString) {
    switch (colorString) {
        case ("themeprimary"):
            return themeprimary;
        case ("themesecondary"):
            return themesecondary;
        case ("themethirdcolor"):
            return themethirdcolor;
        case ("themefourthcolor"):
            return themefourthcolor;
        case ("themefifthcolor"):
            return themefifthcolor;
        default:
            return colorString;
    }
}
/*#endregion Get Colors*/


//Switch Classes Function
function switchClasses(firstClass, secondClass) {

    var firstclasses = document.getElementsByClassName(firstClass);

    for (i = firstclasses.length - 1; i >= 0; i--) {
        if (!hasClass(firstclasses[i], 'dropdown-menu')) {
            addClass(firstclasses[i], firstClass + '-temp');
            removeClass(firstclasses[i], firstClass);
        }
    }

    var secondclasses = document.getElementsByClassName(secondClass);

    for (i = secondclasses.length - 1; i >= 0; i--) {
        if (!hasClass(secondclasses[i], 'dropdown-menu')) {
            addClass(secondclasses[i], firstClass);
            removeClass(secondclasses[i], secondClass);
        }
    }

    tempClasses = document.getElementsByClassName(firstClass + '-temp');

    for (i = tempClasses.length - 1; i >= 0; i--) {
        if (!hasClass(tempClasses[i], 'dropdown-menu')) {
            addClass(tempClasses[i], secondClass);
            removeClass(tempClasses[i], firstClass + '-temp');
        }
    }
}


//Add Classes Function
function addClass(elem, cls) {
    var oldCls = elem.className;
    if (oldCls) {
        oldCls += " ";
    }
    elem.className = oldCls + cls;
}

//Remove Classes Function
function removeClass(elem, cls) {
    var str = " " + elem.className + " ";
    elem.className = str.replace(" " + cls, "").replace(/^\s+/g, "").replace(/\s+$/g, "");
}

//Has Classes Function
function hasClass(elem, cls) {
    var str = " " + elem.className + " ";
    var testCls = " " + cls + " ";
    return (str.indexOf(testCls) != -1);
}

