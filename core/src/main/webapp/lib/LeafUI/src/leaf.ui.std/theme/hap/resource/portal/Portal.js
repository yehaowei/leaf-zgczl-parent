...
A.Portal = Ext.extend(A.Component,{
	...
	initEvents:function(){
         ...
         * @event itemclose
         * 窗口关闭事件.
         * @param {Leaf.PortalPanel} portalPanel portal_panel面板对象
         * @param {Leaf.Portal} portal portal对象
			...
			failure: function(response, opts){
				...
				switch(response.status){
			        case 404:
			            ...
			            break;
			        case 500:
			            msg.push('<H2>',response.status , _lang['ajax.error'],'</H2>',_lang['tab.internet.error'],'<a href="javascript:$le(\''+sf.id+'\').load(\''+url+'\')">',_lang['tab.internet.refresh'],'</a>');
			            break;
			        case 0:
			            break;
			        default:
