$L.uploadcmps = [];
$L.HybridUploader = function(config){
    if(config.type == 'resumable'){
        new $L.ResumableUploader(config)        
    }else{
        new $L.HTML5Uploader(config)
    }
}
$L.HTML5Uploader = Ext.extend($L.Component,{
    types:[],
    map:{},
    sequence:1,
    initComponent : function(config){
        $L.HTML5Uploader.superclass.initComponent.call(this, config);
    ...
    processListener: function(ou){
        $L.HTML5Uploader.superclass.processListener.call(this,ou);
    ...
    initEvents: function(){
        $L.HTML5Uploader.superclass.initEvents.call(this);
        this.addEvents( 
            /**
             * @event upload
             * 上传完毕.
    ...
    checkFile : function(files) {
        ...
        if(!checkType) {
            $L.showInfoMessage(_lang['upload.error'], _lang['upload.error.invalid_file_type']+' <br/>('+ this.filetype + ')',null,350,100);
        }else if(!checkSize) {
            $L.showInfoMessage(_lang['upload.error'], _lang['upload.error.size_exceed']+'('+formatFileSize(fl*1024)+ ')',null,350,100);
        }else if(!checkTotal) {
            $L.showErrorMessage(_lang['upload.error'], _lang['upload.limit_exceeded']+'('+formatFileSize(1024 *this.totalfilesize)+')',null,350,100);
        }else if(!checkTotalCount) {
            $L.showErrorMessage(_lang['upload.error'], _lang['upload.error.number_exceed']+'(' + this.totalcount+')',null,350,100);
        }
        return checkType&&checkSize&&checkTotal&&checkTotalCount;
    },
    uploadFiles : function(files){
        if(!this.showupload) {
            $L.showInfoMessage(_lang['upload.error'], _lang['upload.disabled']);
            this.clearDragTip();
            return;
        }
        ...
        for (var i = 0; i < files.length; i++) {  
            ...
            Ext.each($le(this.id+'_ds').data,function(item){
    ...
    uploadProgress: function(evt,fid) {
        if (evt.lengthComputable) {
            try {
                ...
                var record = $le(this.id+'_ds').find('file_id', fid);
                if (record) {
    ...
    uploadComplete: function(evt, fid) {
        ...
        var res = Ext.decode(serverData);
            ...
            if(!isNaN(res)){
                ...
                $le(this.id+'_ds').remove(record);
            }else {
                $L.showErrorMessage(_lang['upload.error'], _lang['upload.error.unknown']);
                $le(this.id+'_ds').remove(record);
            }
        }else {
            $L.showErrorMessage(_lang['upload.error'], evt.target.response||evt.target.responseText,null,500,300);
            $le(this.id+'_ds').remove(record);
        }
    },
    ...
    clearOtherUpload: function(e){
        var sf = this;
        Ext.each($L.uploadcmps, function(uploader){
    ...
    destroy : function(){
        $L.HTML5Uploader.superclass.destroy.call(this);
...
$L.ResumableUploader = Ext.extend($L.HTML5Uploader,{
    
    initComponent : function(config){
        $L.ResumableUploader.superclass.initComponent.call(this, config);
        $L.uploadcmps.add(this);
        this.le = this.wrap.child('#'+this.id+'_list');
        this.text = this.wrap.child('#'+this.id+'_text');
        this.cv = this.wrap.child('#'+this.id+"_cv");
        ...
        r.on('fileAdded', function(file){
            if(file.fileName){
                Ext.each($le(self.id+'_ds').data,function(item){
                    var s = item.get('sequence');
        ...
        r.on('fileSuccess', function(file,message){
            if(file instanceof ResumableChunk){
                var record = $le(self.id+'_ds').find('file_id', file.fileObj.fid);
