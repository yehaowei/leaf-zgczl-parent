...
HlsBarChart.prototype.renderUI = function () {
    ...
    if(opt.hlsDataSource.split(',').length >1){
        ...
        var divEle = $jq("<div></div>");
        divEle.html(html);
        $jq("#" + opt.id).append(divEle);
    }
}

HlsBarChart.prototype.registerClickEvetn = function () {
    var opt = this.opt, self = this;
    $jq("#EVENT_" + opt.id).bind("click", function (e) {
...
HlsPieChart.prototype.renderUI = function () {
    ...
    if (item) {
        ...
        $jq("#" + this.opt.id + "-task-count").append(count);
        if (percent != "" && percent != null) {
            $jq("#" + this.opt.id + "-task-percent-span").append(percent + '%');
        } else {
            $jq("#" + this.opt.id + "-task-percent-span").append('--');
        }
        $jq("#" + this.opt.id + "-row-task-left-circle-canvas").attr("data-percent", percent);
        ...
        $jq("#" + this.opt.id + "-task-percent-span").append('--');
        $jq("#" + this.opt.id + "-task-count").append(0);
        $jq("#" + this.opt.id + "-row-task-left-circle-canvas").attr("data-percent", 0);
    }
}
...
HlsRateSumChart.prototype.renderUI = function () {
    ...
    $jq("#throwTotalAmountId_"+ this.opt.id).html(total);
    $jq("#receivedThrow_"+ this.opt.id).html(part);
    $jq("#noThrow_"+ this.opt.id).html(left);
...
HlsAnalyseCountChart.prototype.renderUI = function () {
    ...
    $jq("#con-percent-color-1_" + this.opt.id + " span").html(datas[0][statusInfo[0].newAddParam]);
    $jq("#con-percent-color-1_" + this.opt.id).css("width", color1Width + "px");
    $jq("#con-percent-color-2_" + this.opt.id).css("width", (681 - color1Width ) + "px");
        ...
        for (var j = 0; j < datas.length; j++) {
            for (var attr in datas[j]) {
                if (statusInfo[i].status == datas[j][attr]) {
                    ...
                    html = html + '</div>';
                    $jq("#body2_" + this.opt.id).append(html);
                    html = "";
                    isExsit = true;
                    break;
                }
...
//工具方法
$jq.extend(HlsAnalyseCountChart.prototype,{
   showTitle: function () {
       $jq('#anylyse-count-top_' + this.opt.id).show();
   },
   hideTitle: function () {
       $jq('#anylyse-count-top_' + this.opt.id).hide();
   }
...
HlsCircularChart.prototype.renderUI = function () {
    ...
    $jq("#show_" + this.opt.id).html(html);
...
HlsSolidPieChart.prototype.renderUI = function () {
    ...
    $jq("#show_solid" + this.opt.id).html(html);
    console.log(pieData);
...
HlsBrokenChart.prototype.renderUI = function () {
    var self = this;
    self.$jqtopCont = $jq("#" + this.opt.id + "-con-overdue-total");
    self.$jqbottomCont = $jq("#" + this.opt.id + "-outDiv");
    self.$jqgraph = $jq("#" + this.opt.id + "-contract-overdue-span");
    self._initTopCont();
    self._initBottomBar();
    ...
    $jq("#" + self.opt.id + "sysData").html(tyearss + "年" + tmonthss + "月" + tday + "日");
}
HlsBrokenChart.prototype._initTopCont = function () {
    ...
    if (datas && datas[0]) {
        ...
        self.$jqtopCont.html(Hls.formatCurrency(sumAcount.toFixed(2)));
    } else {
...
HlsBrokenChart.prototype._initBottomBar = function () {
    ...
    if (days <= 7) {
        ...
        $jq('#' + self.opt.id + '-day-7').text(sysDateStr);
        var previousDate = self.getTheDate(sysDateStr, -(parseInt(days / 7) + 1));
        for (var k = 6; 0 < k; k--) {
            $jq('#' + self.opt.id + '-day-' + k).text(previousDate);
...
HlsLineChart.prototype.renderUI = function () {
    ...
    if (titleLeft) {
        $jq("#" + this.opt.id + "-total-description").html(titleLeft.description || '');
        $jq("#" + this.opt.id + "-total-amount").html(Hls.formatCurrency(tmp));
    }
    if (titleRight) {
        $jq("#" + this.opt.id + "-every-description").html(titleRight.description || '');
        $jq("#" + this.opt.id + "-every-amount").html(Hls.formatCurrency(current));
    }

    $jq("#" + this.opt.id + "-span").attr("data-composite", ykeyArr.join(','));
    $jq("#" + this.opt.id + "-span").text(ykeyTotalArr.join(','));
...
HlsFoldLineChart.prototype.renderUI = function () {
    ...
    if (data.length > 1) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (title != null && title != "") {
                $jq("#" + self.opt.titleId).html(title);
            }
        }
    }
    else {
        $jq("#" + self.opt.titleId).html(data[0][title]);
    }
    $jq("#account_first_title_color").css("background", color);
...
HlsInfoSquareFrame.prototype.renderUI = function () {
    ...
    html = html + '</div>';
    $jq('#' + this.opt.id).empty().append(html);
}
...
HlsPercentChart.prototype.renderUI = function () {
    ...
    if (datas[0]) {
        if (percent != undefined) {
            ...
            if (radio != null || radio != undefined) {
                $jq("#percent-span" + id).append(radio + '%');
            } else {
                $jq("#percent-span" + id).append('--');
            }
            $jq('#circle-canvas' + id).attr("data-percent", radio);
            $jq('#circle-canvas' + id).attr("data-trackcolor", this.opt.trackColor);
    ...
    if (items) {
        ...
        $jq("#" + this.opt.id).append(appendHtml);
    }
}
