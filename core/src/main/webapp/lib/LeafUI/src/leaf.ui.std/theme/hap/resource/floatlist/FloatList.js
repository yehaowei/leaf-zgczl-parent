(function ($L) {
    ...
    EACH = Ext.each,
	EVT_LOAD = 'load';
	$L.FloatList = Ext.extend($L.Component, {
...
//
			...
			sf.rowArr = [];
			sf.cellArr = [];
			$L.FloatList.superclass.constructor.call(sf, config);
		},
		initComponent: function (config) {
			$L.FloatList.superclass.initComponent.call(this, config);
			sf = this;
...
//			sf.timer = null;
		...
		processListener: function (ou) {
			var sf = this;
			$L.FloatList.superclass.processListener.call(sf, ou);
			if(sf.loadMore){
		...
		initEvents: function () {
			$L.FloatList.superclass.initEvents.call(this);
			this.addEvents(
					/**
					 * @event beforecloserow
					 * 行上的关闭按钮点击时触发该事件
					 * @param {Leaf.FloatList} floatList 当前FloatList组件.
					 * @param {Leaf.Record} record 点击关闭按钮所在行的Record对象.
					 ...
					 * 加载更多之前触发的事件
					 * @param {Leaf.FloatList} floatList 当前FloatList组件.
					 * @param {Leaf.DataSet} dataSet 当前FloatList所绑定的DataSet
					 ...
					 * @event rowclick
					 * 加载更多触发的事件
					 * @param {Leaf.HList} HList 当前HList组件.
					 * @param {Leaf.Record} record 点击关闭按钮所在行的Record对象.
					 * @param {Dom} rowElement 当前行的dom元素
					 */
					EVT_ROW_CLICK,
			...
			Ext.each(records, function (record) {
				...
				Ext.each(sf.columns, function (column) {
					...
					if (column.renderer) {
						...
						cellContent = $L.escapeHtml(value);
					}
					/**
					 * author:徐昭
					 * time:2017/8/14
...
//	    	sh = 0;
	    ...
	    showExportConfirm :function(){
	        ...
	        sf.exportwindow = $L.showOkCancelWindow(_lang['grid.export.config'],msg.join(_N),function(win2){
...
//
	        ...
	        $L.doExport(sf.ds,sf.exportColumns,NULL,opt.type,opt.separator,opt.filename,opt.param);
...
//				},300);
		...
		doMask : function(){
			var sf = this;
			Leaf.Masker.mask(sf.wrap);
		},
		doUnMask : function(){
			var sf = this;
			Leaf.Masker.unmask(sf.wrap);
		},
		/*
		 * 徐昭 2017/11/14 13:40
