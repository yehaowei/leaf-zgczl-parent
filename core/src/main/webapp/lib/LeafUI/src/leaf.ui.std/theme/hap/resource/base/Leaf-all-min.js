...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
                if(svc[0] == '/'){
                    url = ctx + 'sys_lov_common.lview?url=' + encodeURIComponent(ctx + sf['controller'], Ext.urlEncode(sf.getLovPara())) + '&service=' + encodeURIComponent(svc);
                }else {
            ...
            if (url) {
                ...
                sf.win = new A.Window({
                    title: sf.title || 'Lov',
                    url: Ext.urlAppend(url, "lovid=" + sf.id + "&key=" + encodeURIComponent(v) + "&gridheight=" + Math.max(sf.lovgridheight || 355, 355) + "&innerwidth=" + (w - 15) + "&lovautoquery=" + (Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery) + "&lovlabelwidth=" + Math.max(sf.lovlabelwidth || 110, 110) + "&lovpagesize=" + (sf.lovpagesize || '')),
                    height: h,
                    width: w
                });
