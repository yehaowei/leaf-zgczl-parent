...
function deleteFileRecord(did,id) {
    ...
    Leaf.showConfirm(_lang['window.button.ok'],_lang['upload.delete.sure'],function(win){
        win.close();
        ds.remove(record);
        var upload = did.substring(0,did.indexOf('_ds'));
        $le(upload).fireEvent("delete", this);
    })
}

function upFileRecord(did,id) {
    var ds = $le(did),record = ds.findById(id),i = ds.data.indexOf(record);    
    if(i!=-1&& i!=0){
        ...
        $L.request({url:window.window._context_path + '/atm_reorder.svc', para:ds.getJsonData(), success:function(res){
...
function cancelUpload(did,id) {
	...
	var record = ds.findById(id);
	if (record) {
        if($L.uploadcmps && $L.uploadcmps.length >0){
...
function processPercent(record,canDelete,reorder) {
	...
	if (Ext.isEmpty(percent) || status == 1) {
		html += '<div style="float:left;margin-left:10px;color:#808080">'
                ...
                + Leaf.formatDateTime(record.get('creation_time'))
				+ '</div>'
...
$L.UploadList = Ext.extend($L.Component,{
    initComponent : function(config){
        $L.UploadList.superclass.initComponent.call(this, config);
        this.drawList();
    },
    processListener: function(ou){
        $L.UploadList.superclass.processListener.call(this,ou);
        var sf = this,bt = sf.bindtarget,ds = $L.CmpManager.get(bt)
        if(ds){
    ...
    onUpdate : function(ds,record, name, value){
        if(name=='percent' || name == 'status') {
            ...
            if (Ext.isEmpty(percent) || status == 1) {
                ...
                this.wrap.child("#atm_"+record.id).child('.k').update(formatFileSize(record.get('file_size'))+'<span class="d">'+Leaf.formatDateTime(record.get('creation_time'))+'</span>');
            } else if (percent == -1) {
    ...
    drawList: function(){
        ...
        Ext.each(ds.getAll(),function(record){
            ...
            html.push('<div class="k">'+formatFileSize(record.get('file_size'))+'<span class="d">'+Leaf.formatDateTime(record.get('creation_time'))+'</span></div>');
            html.push('</div>')            
        },this)
