...
$L.Lov = Ext.extend($L.TextField,{
    ...
    constructor: function(config) {
        ...
        $L.Lov.superclass.constructor.call(this, config);        
    },
    initComponent : function(config){
        $L.Lov.superclass.initComponent.call(this,config);
        this.para = {};
        ...
        if(this.autocomplete = this.autocomplete == "true"){
        	...
        	this.autocompleteview = new $L.Popup({});
        	if(!this.optionDataSet)this.optionDataSet = new $L.DataSet({id:this.id+"_autocomplete_ds",autocount:false})
        }
    ...
    processListener: function(ou){
        $L.Lov.superclass.processListener.call(this,ou);
        this.trigger[ou]('click',this.onTriggerClick, this, {preventDefault:true})
    },
    initEvents : function(){
        $L.Lov.superclass.initEvents.call(this);
        this.addEvents(
        /**
         * @event commit
         * commit事件.
    ...
    destroy : function(){
        ...
        $L.Lov.superclass.destroy.call(this);
    },
    setWidth: function(w){
...
//				            url = sf.context + 'sys_lov.lsc?svc='+sf.lovservice +'&'+ Ext.urlEncode(sf.getLovPara());
    ...
    onKeyDown : function(e){
        ...
        if(this.autocomplete && this.autocompleteview && this.autocompleteview.isShow){
            ...
            $L.Lov.superclass.onKeyDown.call(this,e);
        }
    },
    onFocus : function(e){
    	if(this.autocomplete){
    	...
    	$L.Lov.superclass.onFocus.call(this,e);
    },
    onBlur : function(e){
    	if(this.autocomplete){
    		if(this.showCompleteId){
    	...
    	$L.Lov.superclass.onBlur.call(this,e);
    },
    ...
    autoCompletePosition:function(){
    	var xy = this.wrap.getXY(),
			...
			BH=$L.getViewportHeight()-3,BW=$L.getViewportWidth()-3,
    ...
    getRenderText : function(record,displayFields){
        var rder = $L.getRenderer(this.autocompleterenderer);
        var text = '&#160;';
        if(rder){
...
//      $L.Lov.superclass.setValue.call(this, v, silent);
...
//            url = this.context + 'sys_lov.lsc?svc='+this.lovservice+'&pagesize=1&pagenum=1&_fetchall=false&_autocount=false&'+ Ext.urlEncode(this.getLovPara());
        ...
        this.qtId = $L.request({url:url, para:p, success:function(res){
            var r = new $L.Record({});
            if(res.result.record){
                ...
                if(l>0){
                	if(this.fetchsingle && l>1){
            			...
            			for(var i=0;i<l;i++){
							var text = this.getRenderText(new $L.Record(datas[i]),displayFields);
							sb.add('<tr tabIndex="'+i+'"'+(i%2==1?' class="autocomplete-row-alt"':'')+'>'+text+'</tr>');	//this.litp.applyTemplate(d)等数据源明确以后再修改		
						}
						sb.add('</table>');
						var div = new Ext.Template('<div style="position:absolute;left:0;top:0">{sb}</div>').append(document.body,{'sb':sb.join('')},true),
                			cmp = new $L.Window({id:this.id+'_fetchmulti',closeable:true,title:'请选择', height:Math.max(div.getHeight(),300),width:Math.max(div.getWidth(),200)});
                		...
                		cmp.body.on('dblclick',function(e,t){
							...
							if(index<-1)return;
							var r2 = new $L.Record(datas[index]);
...
//            url = this.context + 'sys_lov.lview?url='+encodeURIComponent(this.context + 'sys_lov.lsc?svc='+this.lovservice + '&'+ Ext.urlEncode(this.getLovPara()))+'&service='+this.lovservice+'&';
            url = this.context + 'sys_lov.lview?url='+encodeURIComponent(this.context + 'autocrud/'+this.lovservice+'/query'+ (!Ext.isEmpty(lp) ? '?' + lp : ''))+'&service='+this.lovservice+'&';
        }else if(!Ext.isEmpty(this.lovmodel)){
            url = this.context + 'sys_lov.lview?url='+encodeURIComponent(this.context + 'autocrud/'+this.lovmodel+'/query'+ (!Ext.isEmpty(lp) ? '?' + lp : ''))+'&service='+this.lovmodel+'&';
        }
        if(url) {
	        this.isWinOpen = true;
	        //alert(this.lovlabelwidth+' '+this.lovgridheight)
            this.win = new $L.Window({title:this.title||'Lov', url:url+"lovid="+this.id+"&key="+encodeURIComponent(v)+"&gridheight="+(this.lovgridheight||350)+"&innerwidth="+((this.lovwidth||400)-30)+"&lovautoquery="+this.lovautoquery+"&lovlabelwidth="+this.lovlabelwidth, height:this.lovheight||400,width:this.lovwidth||400});
            this.win.on('close',this.onWinClose,this);
        }
    }
});

$L.Popup = Ext.extend($L.Component,{
	constructor : function(config) {
		var id = 'leaf-item-popup',popup = $L.CmpManager.get(id);
		if(popup)return popup;
		config.id=id;
        $L.Popup.superclass.constructor.call(this, config);
    },
    initComponent : function(config){
        $L.Popup.superclass.initComponent.call(this,config);
    ...
    initEvents : function(){
        $L.Popup.superclass.initEvents.call(this);
        this.addEvents(
        	'show',
        	'hide',
        	'beforerender',
    ...
    destroy : function(){
    	$L.Popup.superclass.destroy.call(this);
    	this.processDataSet('un');
    	delete this.shadow;
    },
