$L.Comment = Ext.extend($L.Component, {
	tpl : [
			...
			"</div>",
			"<div class='comment-bar'><a href='javascript:$le(\"{id}\").remove(\"{commentid}\")'>删除</a></div>",
			"</div>", "</li>"],
	maskTpl : [
			"<div class='comment-login-mask' >",
			"<div unselectable='on' class='comment-mask'></div>",
			"<div unselectable='on' class='comment-login'><p>匿名用户不能发表评论</p><p><a href='javascript:$le(\"{id}\").login()'>登录</a> | <a href='javascript:$le(\"{id}\").register()'>注册</a></p></div>",
			"</div>"],
	initComponent : function(config) {
		$L.Comment.superclass.initComponent.call(this, config);
		this.txt = $le(this.id + "_txt");
	...
	post : function() {
		...
		if (Ext.isEmpty(txt)) {
			$L.showErrorMessage("错误", "请输入评论内容", function() {
				textarea.focus();
			});
			return;
		}
		$L.Masker.mask(this.wrap, "正在发布...");
		$L.request({
	...
	remove : function(commentId) {
		var sf = this;
		$L.showConfirm("删除", "是否确定删除？", function() {
			$L.Masker.mask(sf.wrap, "正在删除...");
			$L.request({
