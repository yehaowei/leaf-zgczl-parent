...
$L.MenuBar=Ext.extend($L.Component,Ext.apply({
	...
	processListener: function(ou){
    	$L.MenuBar.superclass.processListener.call(this,ou);
    ...
    renderText : function(data){
    	...
    	return $L.getRenderer(this.renderer).call(window,data.get(this.displayfield),data,this.context);
    },
    ...
    directURL : function(url,title){
    	...
    	else new $L.Window({title:title,url:url,width:Ext.fly(document).child('html').getWidth()-100,height:Ext.fly(document).child('html').getHeight()-100})
    },
    sortOptions : function(o1,o2){
    	if(o1.options)o1.options.sort($L.MenuBar.prototype.sortOptions);
    	if(o2.options)o2.options.sort($L.MenuBar.prototype.sortOptions);
	...
	addMenus : function(options){
		for(var i=0,j=this.children.length;i<options.length;i++){
			...
			menu=new Leaf[options[i].options?'Menu':'MenuItem'](Ext.apply(options[i],{id:_id,parent:this,bar:this,index:j}));
	...
	destroy : function(){
		...
		$L.Menu.superclass.destroy.call(this);
	},
 ...
 * @param {Object} config 配置对象. 
 */
$L.MenuItem=Ext.extend($L.Component,Ext.apply({
	constructor: function(config) {
		this.hasIcon=false;
		$L.MenuItem.superclass.constructor.call(this, config);
	},
	initComponent : function(config){
		$L.MenuItem.superclass.initComponent.call(this,config);
	...
	processListener: function(ou){
    	$L.MenuItem.superclass.processListener.call(this,ou);
	...
	initEvents : function(){
		$L.MenuItem.superclass.initEvents.call(this);
		this.addEvents(
		/**
         * @event submit
         * menu的url定向.
	...
	destroy : function(){
		delete this.el;
		delete this.bar;
		$L.MenuItem.superclass.destroy.call(this);
	},
...
/**
 * @class Leaf.Menu
 * @extends Leaf.MenuItem
 * <p>树形组件.
 * @author huazhen.wu@hand-china.com
 * @constructor
 * @param {Object} config 配置对象. 
 */
$L.Menu=Ext.extend($L.MenuItem,{
	constructor: function(config) {
		this.children=[],this.selectIndex=null,this.groups={},this.isActive=false,this.initMenus=false;
		$L.Menu.superclass.constructor.call(this, config);
	},
	initComponent : function(config){
		$L.Menu.superclass.initComponent.call(this,config);
	...
	addMenus : function(options){
		...
		for(var i=0,j=this.children.length;i<options.length;i++){
			...
			menu=new Leaf[options[i].options?'Menu':'MenuItem'](Ext.apply(options[i],{id:_id,parent:this,bar:this.bar,index:j}));
			if(menu.groupName){
	...
	hide : function(){
		$L.Menu.superclass.hide.call(this);
