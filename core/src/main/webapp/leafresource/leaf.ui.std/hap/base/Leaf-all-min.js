...
//
    ...
    var token = $jq('meta[name=_csrf]').attr('content');
    if (url) form.action = url + (url.indexOf('?') == -1 ? '?' : '&') + 'r=' + Math.random()+"&_csrf="+token;
        ...
        if (fire !== false && issubValid !== false) {
            ...
            if (!this.isValid) {
                ...
                for (var key in valid) {
                    ...
                    if(valid[key]=='请输入必输字段:' && key.substring(key.length-2) =='_n' ){
                        var str =key.substring(0,key.length-2);
                        if (!$L.isEmpty(valid[str])){
                            continue;
                        }
                    }
...
// }
    ...
    A.showYesNoCancelWindow = function (title, msg, yesfun, nofun, width, height) {
        ...
        if (!Ext.isEmpty(msg, true)) {
            msg = '<div class="win-icon win-question"><div class="win-type" style="width:' + (width - 70) + 'px;max-height:' + (height - 55) + 'px;min-height: 60px">' + msg + '</div></div>';
            cmp.body.update(msg + '<center style="padding:5px;padding-bottom: 24px;padding-right: 28px"><table cellspacing="5"><tr><td style="padding-left:5px;padding-right:5px;">' + yesbtnhtml + '</td><td style="padding-left:5px;padding-right:5px;">' + nobtnhtml + '</td><td>' + cancelbtnhtml + '</td></tr></table></center>', true, function () {
                var yesbtn = $le(yesid),
                    nobtn = $le(noid),
...
// sf.setRawValue(_N);
        ...
        showLovWindow: function () {
            ...
            if (sf.lovheight&&sf.lovheight >= h) {
                h = sf.lovheight;
            }
...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
                if(svc[0] == '/'){
                    url = ctx + 'sys_lov_common.lview?url=' + encodeURIComponent(ctx + sf.controller.substr(1), Ext.urlEncode(sf.getLovPara())) + '&service=' + encodeURIComponent(svc) + '&dto=' + encodeURIComponent(sf['dto']);
                }else {
            ...
            if (url) {
                ...
                sf.win = new A.Window({
                    title: sf.title || 'Lov',
                    url: Ext.urlAppend(url, "lovid=" + sf.id + "&key=" + encodeURIComponent(v) + "&gridheight=" + Math.max(sf.lovgridheight || girdH, girdH) + "&innerwidth=" + (w - 15) + "&lovautoquery=" + (Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery) + "&lovlabelwidth=" + Math.max(sf.lovlabelwidth || 110, 110) + "&lovpagesize=" + (sf.lovpagesize || '')),
                    height: h,
                    width: w
                });
...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
            ...
            if (showCode) {
                url = ctx + 'sys_multiLov.lview?url=' + encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/' + svc + '/query', Ext.urlEncode(sf.getLovPara()))) + '&service=' + svc + '&fieldRecord=' + JSON.stringify(fieldRecord) + '&showCode=' + showCode;
            } else {
