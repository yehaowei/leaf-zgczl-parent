...
function getConContractDueAmount(id, contextPath) {
    ...
    Leaf.request({
        url: $('bmLink_home_contract_query').getUrl(),
        scope: this,
        success: function (res) {
            ...
            if (datas && datas.length) {
                ...
                datas = [
                    {
                        sumDueAmount: 466078.38,
                        sumPrincipal: 417738.20,
                        sumInterest: 48340.18,
                        due_date: "2018-01-05 00:00:00"
                    }
                ];
            }
            //获取Y轴最大值
            ...
            $jq('#' + id).bind('plotclick', function (event, ranges) {
...
function draw_block_pi(id, res, maxX, maxY) {
    plot = $jq.plot("#" + id, getSeriesObj(res, 0), {
        yaxis: {
            color: '#f3f3f3',
            min: 0,
            max: 100,
